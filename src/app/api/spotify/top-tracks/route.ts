...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (limit <= 50) {
      ...
      const first = await spotifyApi<{ items: SpotifyTrack[] }>(
        `/me/top/tracks?time_range=${timeRange}&limit=50&offset=0`,
      );
      const second = await spotifyApi<{ items: SpotifyTrack[] }>(
        `/me/top/tracks?time_range=${timeRange}&limit=${limit - 50}&offset=50`,
      );
